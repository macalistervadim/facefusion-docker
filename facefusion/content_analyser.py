            ...
            "hashes": {
                "content_analyser": {
                    ...
                    "path": resolve_relative_path(
                        "../.assets/models/yolo_11m_nsfw.hash",
                    ),
                },
            },
            "sources": {
                "content_analyser": {
                    ...
                    "path": resolve_relative_path(
                        "../.assets/models/yolo_11m_nsfw.onnx",
                    ),
                },
            },
            "size": (640, 640),
...
def get_inference_pool() -> InferencePool:
    ...
    return inference_manager.get_inference_pool(
        __name__,
        model_names,
        model_source_set,
    )
...
def pre_check() -> bool:
    ...
    return conditional_download_hashes(
        model_hash_set,
    ) and conditional_download_sources(model_source_set)
...
def forward(vision_frame: VisionFrame) -> Detection:
    ...
    with conditional_thread_semaphore():
        detection = content_analyser.run(
            None,
            {
                "input": vision_frame,
            },
...
def prepare_detect_frame(temp_vision_frame: VisionFrame) -> VisionFrame:
    ...
    detect_vision_frame = numpy.expand_dims(
        detect_vision_frame.transpose(2, 0, 1),
        axis=0,
